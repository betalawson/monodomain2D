...
% Read out the ischaemia parameters
hypoxia_factor = params(1);
hyperkalemia_factor = params(2);
acidosis_factor = params(3);
...
K_o =  5.4 + (K_o_max - 5.4) * hyperkalemia_factor;    % Extracellular K+ concentration (base)
Ca_o = 2;                                              % Extracellular Ca2+ concentration
Na_i = 7.67;                                           % Intracellular Na+ concentration (fixed in reduced model)
K_i = 138.3 - 13.3 * acidosis_factor;                  % Intracellular K+ concentration (fixed in reduced model)
...
% Apply acidosis
V_mod = 3.4 * acidosis_factor;                         % Modification of reversal potentials
g_Na = g_Na * (1 - 0.25 * acidosis_factor);            % Reduction in I_Na due to pH change
g_CaL = g_CaL * (1 - 0.25 * acidosis_factor);          % Reduction in I_Na due to pH change
k_NaCa = k_NaCa * (1 - 0.25 * acidosis_factor);        % Reduction in I_Na due to pH change
